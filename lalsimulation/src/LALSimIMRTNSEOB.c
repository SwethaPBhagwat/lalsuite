                      ...
                      )
{
  ...
  flux = XLALSimIMRTNSEOBFactorizedFlux( &valuesVec, omega, params, lMax );
                          ...
                          )
{
 ...
 {rstop=4.0;
  printf("We have already reached the distance of 4.0 M and the potential is NOT yet zero. Stop here ANYWAYS!");
  }
...
//   printf("nStepBack");
  ...
  // printf("\n%f",r);
  // printf("Initial r is solved at this point.   r = XLALDBisectionFindRoot( XLALrOfOmegaP4PN, rInitMin, rInitMax, xacc, &TNSpr3In); ENDED. ");
...
{
   fprintf(fp1,"\n%f\t%f", (double) dynamics->data[cellno], (double) rVec.data[cellno]);
}
...
fp2=fopen("Initial_cond_for_highres.txt", "w");
if(fp2 == NULL)
    exit(-1);
//int cellno;
//for ( cellno=0; cellno<retLen; cellno=cellno+1 )
{  fprintf(fp2,"\n%u",lengthHiSR);
   fprintf(fp2,"\n%f",dt/m);
   fprintf(fp2,"\n%f\t%f\t%f\t%f", (double) values->data[0], (double) values->data[1],(double) values->data[2],(double) values->data[3]);
 fprintf(fp2,"\n%u",hiSRndx);
  fprintf(fp2,"hiSRndx");
   fprintf(fp2,"\n%u",retLen);
   fprintf(fp2,"retLen");
  fprintf(fp2,"\n%u",nStepBack);
   fprintf(fp2,"nStepBack");
}
...
for ( cellno=0; cellno<retLen; cellno=cellno+1 )
{
   fprintf(fp,"\n%f\t%f\t%f\t%f", (double) dynamicsHi->data[cellno], (double) rVecHi.data[cellno], (double)phiVecHi.data[cellno], (double) pPhiVecHi.data[cellno] );
}
